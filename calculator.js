...
class ExpressionEstimator{
	...
	*/
	...
	compile(expression, ...a) {
		...
		e = ExpressionEstimator.replaceRarelyFunction(e);
		ExpressionEstimator.rf.forEach(s => {
			k = s.indexOf('(');
		...
		e = e.replace(/\b0[bx][\w\.]+/gi, (e)=>{
			...
			radix=e[1].toLowerCase()=='b'?2:16;
			r = radix==2 ? /^[01]*$/ : /^[\da-fA-f]*$/; 
			v=0;
			g.forEach( (e,i) => {
				if (!e.match(r)) {
					throw new Error("Invalid number " + e);
				}
				j=e.length ? parseInt(e,radix) : 0
				if(i==1){
					j/=radix**e.length;
				}
				v+=j;
		...
		s = s.replace(/\b0[xb][\w\.]+/gi, '')//regex like on line 177
		//complie("0b") output unkonwn varaible "0b" not "b", so use \d* at the beginnning
	...
	static{
		...
		// ExpressionEstimator.rf1 = ExpressionEstimator.rf.filter( e=> {
		// 	return e.indexOf('(')==-1 || !ExpressionEstimator.rarely.includes( e.substr(0,e.length-2));
		// } );
	}
